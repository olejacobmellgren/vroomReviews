...
const Favoritepage = () => {
  ...
  const userFavorites = allreviews.filter((review) => review?.userID === "1" && review?.isFavorite === true).map((review) => review?.carID);
  // Get all cars that are favorited by user
  const favoriteCars = cars.filter((car) => car?.id && userFavorites.includes(car?.id));
